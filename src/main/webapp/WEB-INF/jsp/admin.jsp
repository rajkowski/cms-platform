<%--
  ...
  --%>
<%@ page import="static com.zeroio.platform.ApplicationInfo.PRODUCT_NAME" %>
  ...
  <div class="off-canvas position-left reveal-for-medium admin-menu hide-for-print" style="z-index: 1005 !important; padding-bottom: 50px" id="offCanvas" data-off-canvas>
    ...
    <c:if test="${userSession.hasRole('admin') || userSession.hasRole('content-manager')}">
      <ul class="vertical menu">
        ...
        <li<c:if test="${fn:startsWith(pageRenderInfo.name, '/admin/sitemap')}"> class="is-active"</c:if>><a href="javascript:void(0)" onclick="loadAdminContent('${ctx}/admin/sitemap', this)"><i class="${font:far()} fa-sitemap fa-fw"></i> <span>Navigation</span></a></li>
