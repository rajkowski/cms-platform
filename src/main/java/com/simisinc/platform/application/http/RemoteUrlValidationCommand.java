...
public class RemoteUrlValidationCommand {
   ...
   * to precisely those bytes instead of letting it re-resolve the host.
   */
   ...
   * address(es) that were validated, so a caller can pin a subsequent connection to them.
   * Fails closed identically: a blank, malformed, non-http(s), unresolvable, or
