...
public class CheckItemFolderPermissionCommand {
  ...
  public static boolean userHasAddPermission(long folderId, long userId) {
    ...
    SqlWhere where = DB.WHERE()
        .add("user_id = ?", userId)
        .add("group_id IN (SELECT group_id FROM item_folder_groups WHERE folder_id = ? AND add_permission = true)", folderId);
  ...
  public static boolean userHasViewPermission(long folderId, long userId) {
    ...
    SqlWhere where = DB.WHERE()
        .add("user_id = ?", userId)
        .add("group_id IN (SELECT group_id FROM item_folder_groups WHERE folder_id = ? AND view_permission = true)", folderId);
    long count = DB.selectCountFrom("user_groups", where);
    return (count > 0);
  }
  ...
  public static boolean userHasEditPermission(long folderId, long userId) {
    ...
    SqlWhere where = DB.WHERE()
        .add("user_id = ?", userId)
        .add("group_id IN (SELECT group_id FROM item_folder_groups WHERE folder_id = ? AND edit_permission = true)", folderId);
    long count = DB.selectCountFrom("user_groups", where);
    return (count > 0);
  }
  ...
  public static boolean userHasDeletePermission(long folderId, long userId) {
    ...
    SqlWhere where = DB.WHERE()
        .add("user_id = ?", userId)
        .add("group_id IN (SELECT group_id FROM item_folder_groups WHERE folder_id = ? AND delete_permission = true)", folderId);
    long count = DB.selectCountFrom("user_groups", where);
    return (count > 0);
  }
}
