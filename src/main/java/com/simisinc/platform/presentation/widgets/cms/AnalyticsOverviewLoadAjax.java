...
/**
 * Returns analytics overview data including KPIs and trends
 *
 * @author matt rajkowski
 * @created 01/31/26 02:00 PM
 */
public class AnalyticsOverviewLoadAjax extends GenericWidget {

  static final long serialVersionUID = -8484048371911908893L;
  private static Log LOG = LogFactory.getLog(AnalyticsOverviewLoadAjax.class);
  ...
  public WidgetContext execute(WidgetContext context) {
    ...
    if (rangeStart == null || rangeEnd == null) {
      LOG.error("Invalid date range for analytics overview: " + range);
      context.setJson("{\"success\": false, \"error\": \"Invalid date range\"}");
    ...
    try {
      ...
      ObjectNode response = AnalyticsDataService.loadOverview(rangeStart, rangeEnd);
      context.setJson(response.toString());
    } catch (Exception e) {
      LOG.error("Error loading overview analytics", e);
      context.setJson("{\"success\": false, \"error\": \"Failed to load analytics\"}");
      context.setSuccess(false);
    }
