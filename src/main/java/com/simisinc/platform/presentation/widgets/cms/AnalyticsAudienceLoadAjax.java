...
/**
 * AJAX endpoint for analytics audience data
 *
 * @author matt rajkowski
 * @created 01/31/26 02:00 PM
 */
public class AnalyticsAudienceLoadAjax extends GenericWidget {
  static final long serialVersionUID = -8484048371911908895L;
  private static Log LOG = LogFactory.getLog(AnalyticsAudienceLoadAjax.class);
  ...
  public WidgetContext execute(WidgetContext context) {
    ...
    try {
      ...
      ObjectNode response = AnalyticsDataService.loadAudience(rangeStart, rangeEnd);
      context.setJson(response.toString());
    } catch (Exception e) {
      LOG.error("Error loading audience analytics", e);
      context.setJson("{\"success\": false, \"error\": \"Failed to load audience analytics\"}");
